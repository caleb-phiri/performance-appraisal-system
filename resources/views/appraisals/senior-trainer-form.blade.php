...
<head>
    ...
    <title>Senior Trainer Performance Appraisal - MOIC</title>
...
<body class="bg-gray-50">
    @php
        ...
        // Get quarter info with grace period
    ...
    <div class="max-w-7xl mx-auto py-6 px-4">
        <!-- Header -->
        <div class="bg-gradient-to-r from-purple-800 to-indigo-800 rounded-xl shadow-lg mb-6">
            <div class="p-6">
                <div class="flex justify-between items-center">
                    <div>
                        <h1 class="text-2xl font-bold text-white">Senior Trainer Performance Appraisal</h1>
                        <p class="text-purple-100 mt-1">{{ $quarter }} {{ $year }}</p>
                    </div>
                    <a href="{{ route('appraisals.create') }}" class="bg-white text-purple-800 px-4 py-2 rounded-lg hover:bg-purple-50">
        ...
        <div class="bg-white rounded-xl shadow-md p-6 mb-6">
            <div class="grid md:grid-cols-4 gap-4">
                ...
                <div><label class="text-sm text-gray-500">Department</label><p class="font-semibold">Human Resources - Training</p></div>
                <div><label class="text-sm text-gray-500">Review Period</label><p class="font-semibold">{{ $quarterMonths }} {{ $year }}</p></div>
            </div>
            <div class="grid md:grid-cols-4 gap-4 mt-3 pt-3 border-t">
                <div><label class="text-sm text-gray-500">Deadline</label><p class="font-semibold {{ $isInGrace ? 'text-amber-600' : 'text-red-600' }}">{{ $dueDateFormatted }}, {{ $year }} @if($isInGrace)<span class="text-xs bg-amber-100 text-amber-800 px-2 py-1 rounded ml-2">Grace Period</span>@endif</p></div>
            </div>
        </div>
        ...
        <form id="appraisalForm" method="POST" action="{{ route('appraisals.store') }}">
            ...
            <input type="hidden" name="job_title" value="Senior Trainer">
                        ...
                        <tbody>
                            ...
                            <tr class="border-b hover:bg-gray-50">
                                <td class="p-3 font-semibold">
                                    {{ $title }}
                                    <input type="hidden" name="kpas[{{ $index }}][category]" value="SENIOR TRAINING">
            ...
            <div class="bg-white rounded-xl shadow-md p-6 flex justify-between items-center">
                <div class="text-sm text-gray-500">
                    ...
                    Rate each KPA based on training leadership, strategic impact, and team development
                </div>
                <div class="flex gap-3">
                    <button type="button" onclick="saveDraft()" class="bg-yellow-500 hover:bg-yellow-600 text-white px-6 py-2 rounded-lg">
                        <i class="fas fa-save mr-2"></i> Save Draft
                    </button>
                    <button type="button" onclick="submitForm()" class="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2 rounded-lg">
