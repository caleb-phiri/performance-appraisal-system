...
<head>
    ...
    <title>Edit Leave Application - MOIC Performance Appraisal System</title>
            ...
            <div class="flex justify-between items-center h-16">
                ...
                <div class="flex items-center">
                    ...
                    <div class="hidden md:flex items-center">
                        ...
                        <div>
                            <h1 class="text-sm font-bold mb-0">Edit Leave</h1>
                            <p class="mb-0 text-white text-opacity-75 text-xs">#{{ $leave->id }}</p>
                        </div>
                    </div>
    ...
    <div class="max-w-4xl mx-auto px-4 py-8">
        ...
        <div class="mb-6 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative alert-slide" role="alert">
            <div class="flex items-start">
                <div class="bg-red-500 rounded-full p-1 mr-3 mt-1">
                    <i class="fas fa-exclamation-circle text-white text-sm"></i>
                </div>
                <div>
        ...
        <div class="mb-8 text-center">c     
            ...
            <h1 class="text-3xl font-bold" style="color: #110484;">Edit Leave Application</h1>           
            <div class="flex items-center justify-center mt-2">
                <p class="text-gray-600">Update your leave request</p>
                <span class="mx-3 text-gray-300">|</span>
                <span class="font-mono text-sm bg-gray-100 px-3 py-1 rounded-full">#{{ $leave->id }}</span>
            </div>
        </div>

        <!-- Status Warning for Non-Pending Leaves -->
        @if($leave->status !== 'pending')
        <div class="mb-6 warning-alert border border-yellow-400 text-yellow-700 px-6 py-4 rounded-lg relative shadow-sm" role="alert">
            <div class="flex items-start">
                <div class="bg-yellow-500 rounded-full p-2 mr-4">
                    <i class="fas fa-exclamation-triangle text-white text-sm"></i>
                </div>
                <div class="flex-1">
                    <h4 class="font-bold text-yellow-800 mb-1">Application Cannot Be Edited</h4>
                    <p class="text-yellow-700">This leave request is <span class="font-bold uppercase">{{ $leave->status }}</span> and cannot be modified.</p>
                    <a href="{{ route('leave.show', $leave->id) }}" class="inline-flex items-center mt-3 px-4 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-colors text-sm">
                        <i class="fas fa-arrow-left mr-2"></i> Go back to view details
                    </a>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b bg-gradient-to-r from-gray-50 to-gray-100">
                <div class="flex items-center">
                    <div class="w-8 h-8 rounded-full bg-[#110484] flex items-center justify-center mr-3">
                        <i class="fas fa-pen-alt text-white text-sm"></i>
                    </div>
                    <h3 class="text-lg font-bold" style="color: #110484;">Edit Application #{{ $leave->id }}</h3>
                </div>
            </div>
            
            <form action="{{ route('leave.update', $leave->id) }}" method="POST" class="p-6" id="editLeaveForm" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6 flex items-center justify-between bg-gray-50 p-4 rounded-lg border">
                    <div class="flex items-center">
                    ...
                    <div>
                        ...
                        @else
                            <span class="status-badge-cancelled">
                                <i class="fas fa-ban mr-1"></i> {{ ucfirst($leave->status) }}
                            </span>
                        @endif
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="leave_type" id="leave_type" required
                                class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#110484] focus:border-transparent @error('leave_type') border-red-500 @enderror bg-white"
                            ...
                            <option value="compassionate" {{ old('leave_type', $leave->leave_type) == 'compassionate' ? 'selected' : '' }}>Compassionate Leave</option>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="date" name="end_date" id="end_date" required
                                       ...
                                       min="{{ $leave->start_date->format('Y-m-d') }}"
                                       {{ $leave->status !== 'pending' ? 'disabled' : '' }}>
                            </div>
                            @error('end_date')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    ...
                    <div id="daysCount" class="p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border-l-4 border-green-500">
                        <div class="flex items-center">
                            ...
                            <div>
                                <p class="text-sm font-medium text-green-800">
                                    Total Leave Days: <span id="totalDays" class="font-bold text-lg">{{ $leave->total_days }}</span> days
                                </p>
                    ...
                    <div>
                        ...
                        <textarea name="reason" id="reason" rows="4" required
                                  ...
                                  {{ $leave->status !== 'pending' ? 'disabled' : '' }}>{{ old('reason', $leave->reason) }}</textarea>
                        <div class="flex justify-between mt-1">
                            <p class="text-xs text-gray-500">Minimum 10 characters</p>
                            <p id="reasonCounter" class="text-xs text-gray-500">{{ strlen(old('reason', $leave->reason)) }}/500</p>
                        </div>
                        @error('reason')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="p-4 bg-blue-50 rounded-lg border border-blue-200">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                ...
                                <div>
                                    <p class="text-sm font-medium text-gray-700">Current Attachment</p>
                                    <p class="text-xs text-gray-500 mt-1">{{ basename($leave->attachment_path) }}</p>
                                </div>
                            </div>
                            <a href="{{ route('leave.download-attachment', $leave->id) }}" class="text-blue-600 hover:text-blue-800 text-sm">
        ...
        <div class="mt-6 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 border-2 border-blue-100">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="bg-white p-3 rounded-lg shadow-sm">
                    ...
                    <div>
                        @if($leave->status == 'pending')
                            <span class="status-badge-pending inline-flex">
                                <i class="fas fa-clock mr-1"></i> Pending
                            </span>
                        @elseif($leave->status == 'approved')
                            <span class="status-badge-approved inline-flex">
                                <i class="fas fa-check-circle mr-1"></i> Approved
                            </span>
                        @elseif($leave->status == 'rejected')
                            <span class="status-badge-rejected inline-flex">
                                <i class="fas fa-times-circle mr-1"></i> Rejected
                            </span>
                        @else
                            <span class="status-badge-cancelled inline-flex">
                                <i class="fas fa-ban mr-1"></i> {{ ucfirst($leave->status) }}
                            </span>
                        @endif
                    </div>
                ...
                <div class="bg-white p-3 rounded-lg shadow-sm">
                    ...
                    <div class="text-xs text-gray-400">{{ $leave->created_at->format('h:i A') }}</div>
                </div>
                
                @if($leave->approved_at)
                <div class="bg-white p-3 rounded-lg shadow-sm">
                    <div class="text-xs text-gray-500 mb-1">Approved On</div>
                ...
                <div class="bg-white p-3 rounded-lg shadow-sm">
                    <div class="text-xs text-gray-500 mb-1">Rejected On</div>
                ...
                <div class="bg-white p-3 rounded-lg shadow-sm">
                    <div class="text-xs text-gray-500 mb-1">Cancelled On</div>
                    <div class="font-medium text-gray-600 flex items-center">
                        <i class="far fa-calendar-minus mr-1"></i>
                        {{ \Carbon\Carbon::parse($leave->cancelled_at)->format('d/m/Y') }}
                    </div>
                </div>
                @endif
            </div>
        ...
        <div id="loadingOverlay" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
            <div class="bg-white rounded-xl p-8 text-center max-w-sm mx-4">
                ...
                <p class="text-gray-700 font-medium">Updating your application...</p>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (editForm) {
                editForm.addEventListener('submit', function(e) {
                    ...
                    // Show loading overlay if form is valid and pending
                    @if($leave->status === 'pending')
                    if (submitBtn) {
